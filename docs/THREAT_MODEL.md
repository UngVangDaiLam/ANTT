...
| Kẻ dò mật khẩu       | Thử mật khẩu qua API, hoặc brute-force offline sau khi trộm DB                                             | Argon2id `opslimit=3`, `memlimit=64MB` (≈330 ms/lần thử trên máy phát triển) khiến brute-force offline đắt đỏ; rate limit theo IP cho `/register`. **Còn thiếu: ràng buộc độ mạnh mật khẩu; rate limit cho `/login`; và salt giả (D15) tuy đã có hàm `fakeSaltFor` kèm test nhưng chưa route nào gọi** | Trần Bảo  |
...
10. **Metadata lộ gần như toàn bộ** — xem mục 5.
...
| Session token         | CSPRNG (thiết kế: ≥ 128 bit)  | Server, khi đăng nhập                    | SHA-256 của token                       | —            | Hết hạn sau `SESSION.TTL_MS` (24 giờ) hoặc khi đăng xuất              |
